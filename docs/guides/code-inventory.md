...
2. **Test**: Execute all tests (my WP_PHPUnit_Framework, installed in tests/gl-phpunit-test-framework, source in ~/sites/phpunit-testing/) and verify PHPStan compliance. run `php bin/sync-and-test.php` to synchronize code to the WordPress environment and run tests
3. **Commit**: Commit code changes in Git with a descriptive message
4. **Document**: Update documentation if needed, then commit documentation changes separately

### Environment Management
- **Development code** in `~/sites/reinvent/`
- **WordPress environment** is in Lando `~/sites/wordpress/`
- **Reinvent as WordPress Plugin**: Deployed to `~/sites/wordpress/wp-content/plugins/gl-reinvent/` for testing
- **PHPUnit Testing Source Code** in `~/sites/phpunit-testing/`
- **PHPUnit Testing as a Submodule in Reinvent**: ~/sites/reinvent/tests/gl-phpunit-test-framework/
- **Sync**: Use `bin/sync-to-wp.php` for manual syncing
- **Environment variables**: managed via `.env.testing` (copied to WordPress plugin directory)

### DO NOT EDIT (Generated/Deployment Directories)
- `~/sites/wordpress/` - WordPress installation (managed by Lando)
- `~/sites/wordpress/wp-content/plugins/gl-reinvent/` - Deployed plugin (generated by sync script)
- `~/sites/reinvent/tests/gl-phpunit-test-framework/` - Git submodule (update from source instead)
...
### Configuration
- PHPUnit configuration files are located in `tests/bootstrap/`
  - `phpunit-unit.xml.dist` - Unit test configuration
  - `phpunit-wp-mock.xml.dist` - WP-Mock test configuration
  - `phpunit-integration.xml.dist` - Integration test configuration
  - `phpunit-multisite.xml.dist` - Multisite test configuration
  - `phpunit-framework-tests.xml.dist` - Framework test configuration

#### File Inclusion/Exclusion in Tests
- **Base Directory**: All paths are relative to the PHPUnit XML configuration file's location

- **Including Test Files**:
  - Use `<directory>` to specify which directories contain test files
  - Example: `<directory suffix=".php">../Unit</directory>`

- **Excluding Files**:
  - Use `<exclude>` to prevent files from being treated as tests
  - Always exclude base test case classes (i.e. for "class Test_Journey_Questions_Model extends Unit_Test_Case" Unit_Test_Case is a base class, doesn't have any tests itself). Excluding prevents 'No tests found in class' warnings.
  - Example:
    ```xml
    <testsuite name="unit">
...
- **Common Pitfalls**:
  - Abstract base test cases being treated as test classes
  - Framework test files being discovered in the wrong context
  - Test files in vendor directories being included

- **Verification**:
  - Run tests with `-v` to see which files are being discovered
  - Check for warnings about "No tests found in class" which may indicate incorrect exclusions
